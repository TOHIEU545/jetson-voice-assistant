...
"""
M04 - Controlled SNR noise stress benchmark.
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "logs" / "benchmarks" / "stt" / "M04_babble_snr_stress"
...
def load_dataset(manifest_path, snr_levels, limit_per_snr=0):
    ...
    if missing:
        raise RuntimeError(
            "M04 manifest missing columns: %s"
            % ", ".join(sorted(missing))
        )
...
def render_summary(
    ...
    lines = [
        "# M04 — Babble Controlled SNR Stress",
        "",
