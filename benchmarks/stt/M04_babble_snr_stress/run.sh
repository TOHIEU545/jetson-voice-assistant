...
OUTPUT_ROOT="$REPO_ROOT/logs/benchmarks/stt/M04_babble_snr_stress"
...
if [[ ! -f "$RUNNER" ]]; then
    echo "[ERROR] Missing M04 runner: $RUNNER"
    exit 1
fi
if [[ ! -f "$DATASET_DIR/manifest.tsv" ]]; then
    echo "[ERROR] Missing prepared M04 manifest: $DATASET_DIR/manifest.tsv"
...
if missing:
    print("[ERROR] Missing M04 runtime/model files:", file=sys.stderr)
    for path in missing:
