...
if [[ ! -f "$RUNNER" ]]; then
    echo "[ERROR] Missing M02 runner: $RUNNER"
    exit 1
fi
if [[ ! -d "$DATASET_DIR/noisy" || ! -f "$DATASET_DIR/manifest.tsv" ]]; then
    echo "[ERROR] Missing M02 dataset or manifest under: $DATASET_DIR"
    exit 1
fi
...
if missing:
    print("[ERROR] Missing M02 runtime/model files:", file=sys.stderr)
    for path in missing:
...
echo "[INFO] Running Zipformer with GTCRN OFF then ON..."
python3 "$RUNNER" \
