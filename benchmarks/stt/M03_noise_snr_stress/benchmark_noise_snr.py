...
"""
M03 - Controlled SNR noise stress benchmark.
...
Input:
    VoiceBank clean speech
      ...
      + controlled SNR: 20 / 10 / 5 / 0 dB
      -> ALSA Loopback
      -> current project speech runtime
      -> optional GTCRN
      -> Silero VAD
      -> optimized streaming Zipformer pipeline
...
DEFAULT_DATASET = REPO_ROOT / "data" / "stt" / "ms_snsd" / "mixed" / "voicebank_prepared_15" / "airconditioner"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "logs" / "benchmarks" / "stt" / "M03_noise_snr_stress"
...
def load_dataset(manifest_path, snr_levels, limit_per_snr=0):
    ...
    if missing:
        raise RuntimeError(
            "M03 manifest missing columns: %s"
            % ", ".join(sorted(missing))
        )
...
def render_summary(
    ...
    lines = [
        "# M03 — Controlled SNR Noise Stress",
        "",
        "Fixed STT backend: `Zipformer 2023-06-21`",
        "",
        "Noise: `MS-SNSD AirConditioner`",
        "",
        "## Accuracy by SNR",
        "",
        "| SNR | WER OFF | WER ON | ON-OFF | Exact OFF | Exact ON | TOTAL mean OFF | TOTAL mean ON | TOTAL p95 OFF | TOTAL p95 ON |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    lines += [
        "",
        "Negative `ON-OFF` means GTCRN improved WER.",
        "",
        "## Overall",
        "",
        "| Config | Corpus WER | Exact | VAD mean | STT mean | TOTAL mean | TOTAL p95 |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    lines += [
        ...
        "",
        "```text",
        "ALL : improved %d | worsened %d | unchanged %d | paired %d"
        % (
            effect[
                "improved_samples"
            ],
    ...
    for snr_db in snrs:
        ...
        lines.append(
            "%2d dB: improved %d | worsened %d | unchanged %d | paired %d"
            % (
                snr_db,
                bucket.get(
                    "improved_samples",
...
def self_test():
    ...
    # Regression guard:
    # same sample_id at two SNR levels must
    # remain two distinct GTCRN pairs.
    mock = []
...
def main():
    ...
    snr_levels = [
        int(x.strip())
        for x in args.snr.split(",")
        if x.strip()
    ]

    if not snr_levels:
        raise RuntimeError(
            "No SNR levels selected"
        )
    ...
    with samples_path.open(
        ...
        for index, config_key in enumerate(
            order
        ):
            print()
