...
M03 cố định backend `Zipformer 2023-06-21`, so sánh GTCRN OFF/ON trên VoiceBank clean speech trộn deterministic với MS-SNSD `AirConditioner` tại 20/10/5/0 dB.
...
```bash
./benchmarks/stt/M03_noise_snr_stress/run.sh
```
...
- Input: `data/stt/ms_snsd/mixed/voicebank_prepared_15/airconditioner/`
...
```bash
python3 benchmarks/stt/M03_noise_snr_stress/prepare_snr_dataset.py
```
...
python3 benchmarks/stt/M03_noise_snr_stress/benchmark_noise_snr.py \
  --limit-per-snr 1
```
